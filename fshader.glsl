...
void main() {	
	...
	vec4 color_sky = vec4(0.61, 0.46, 0.80, 1);
	...
	vec3 id = max(0.0, dot(sun_dir, norm)) * vec3(0.05, 0.10, 0.10);
	vec3 h = normalize(sun_dir + a);
	vec3 is = specular_strength * pow(max(dot(norm, h), 0.0), specular_power) * specular;
	vec3 i = ia + id + is;
	if (i.x < 0) i.x = 0;
	if (i.y < 0) i.y = 0;
	if (i.z < 0) i.z = 0;
